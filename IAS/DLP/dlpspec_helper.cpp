	...
	switch (data_type)
	{
		case SCAN_DATA_TYPE:
			...
			tn = tpl_map(format_str, struct_p, 
					SCAN_NAME_LEN, NUM_SHIFT_VECTOR_COEFFS, NUM_PIXEL_NM_COEFFS,2,
...
DLPSPEC_ERR_CODE dlpspec_get_scanData_from_slewScanData(const slewScanData *pSlew,
                                                scanData *pData, int section_index, int if_x_shift)
{
    ...
    dlpspec_scan_section_get_adc_data_range(pSlew, section_index, &data_start_index, &num_patterns, &num_black_patterns, if_x_shift);
