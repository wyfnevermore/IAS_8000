		...
		const calibCoeffs *pCoeffs, const FrameBufferDescriptor *pFB, int if_x_shift)
{
	...
	for(i=0; i < pCfg->head.num_sections; i++)
	{
		...
		{
			case COLUMN_TYPE:
				ret_val = dlpspec_scan_col_genPatDef(&cfg, pCoeffs, &patDefC, if_x_shift);
				if (ret_val == DLPSPEC_PASS)
					numPatterns = dlpspec_scan_col_genPatterns(&patDefC, pFB, 
							start_pattern);
				break;
			case HADAMARD_TYPE:
				ret_val = dlpspec_scan_had_genPatDef(&cfg, pCoeffs, &patDefH, if_x_shift);
				if (ret_val == DLPSPEC_PASS)
...
static DLPSPEC_ERR_CODE dlpspec_scan_slew_interpret(uScanData *pData,
		scanResults *pRefResults, int if_x_shift)
{
    ...
    {
        dlpspec_get_scanData_from_slewScanData(&pData->slew_data, &thisScanData.data, i, if_x_shift);
        if(pData->slew_data.slewCfg.section[i].section_scan_type == COLUMN_TYPE)
            ret_val = dlpspec_scan_col_interpret(&thisScanData, &tempScanResults, if_x_shift);
        else if(pData->slew_data.slewCfg.section[i].section_scan_type == HADAMARD_TYPE)
            ret_val = dlpspec_scan_had_interpret(&thisScanData, &tempScanResults, if_x_shift);
        else
 ...
 */
{
    ...
    {
        ret_val = dlpspec_scan_had_interpret((uScanData*)pCopyBuff, pResults, if_x_shift);
    }
    else if(type == COLUMN_TYPE)
    {
        ret_val = dlpspec_scan_col_interpret((uScanData*)pCopyBuff, pResults, if_x_shift);
    }
    else if(type == SLEW_TYPE)
    {
        ret_val = dlpspec_scan_slew_interpret((uScanData*)pCopyBuff, pResults, if_x_shift);
    }
	else
	{
 ...
 */
{
    ...
    */
    ...
    ret_val = dlpspec_scan_interpret(pDesRefScanData, calSize, pRefResults, if_x_shift);
    if (ret_val < 0)
    {
        goto cleanup_and_exit;
    }
 ...
 */
{
    ...
    for(i=0; i<=section_index; i++)
    {
        ...
        else if (pData->slewCfg.section[i].section_scan_type == HADAMARD_TYPE)
        {
            ...
            dlpspec_scan_had_genPatDef(&cfg, &pData->calibration_coeffs, &patDefH, if_x_shift);
