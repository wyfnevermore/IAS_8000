...
// Inclusion Guard
#ifndef _DLPSPEC_SCAN_H_
#define _DLPSPEC_SCAN_H_
...
typedef struct _work_flow_ext {
	uint8_t	sampleobj;			//0---固体，1---液体
	uint8_t lampmode;			//0---内置光源，1---外置光源
	uint8_t	motormode;			//0---电机不转，1---电机转动
}WorkFlowExt,*PWorkFlowExt;
...
typedef struct
{
  ...
  DLPSPEC_ERR_CODE dlpspec_scan_interpret(const void *pBuf, const size_t bufSize,
	   	scanResults *pResults, int if_x_shift);
  ...
  DLPSPEC_ERR_CODE dlpspec_scan_interpReference(const void *pRefCal, 
		...
		const scanResults *pScanResults, scanResults *pRefResults, int if_x_shift);
  int32_t dlpspec_scan_genPatterns(const uScanConfig* pCfg, 
		const calibCoeffs *pCoeffs, const FrameBufferDescriptor *pFB, int if_x_shift);
  ...
  DLPSPEC_ERR_CODE dlpspec_scan_section_get_adc_data_range(const slewScanData 
		*pData, int section_index,int *p_section_start_index, uint16_t *p_num_patterns,
		uint16_t *p_num_black_patterns, int if_x_shift);
