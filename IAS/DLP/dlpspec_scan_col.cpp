...
DLPSPEC_ERR_CODE dlpspec_scan_col_genPatDef(const scanConfig *pScanConfig, 
		const calibCoeffs *pCoeffs, patDefCol *patDef, int if_x_shift)   
/**
 ...
 */
{
	...
	//if(if_x_shift == 1) ////通过if_x_shift判断采集过程是否需要校正，0不校正，1校正。
	if(if_x_shift == 1)
	{
		start_px = start_px + pCoeffs->X_Shift_Data[0];
		end_px = end_px + pCoeffs->X_Shift_Data[1];
	}	
 ...
 */
{
    ...
    */
	...
	if(if_x_shift == 0)
	{
		for(i = 0; i < pResults->length; i++)
		{
			pResults->wavelength[i] = patDef.colMidPix[i];
		}
	...
	}else
	{
		for(i=0; i < pResults->length; i++)
		{
			ret_val = dlpspec_util_columnToNm(patDef.colMidPix[i], 
					&(pScanData->calibration_coeffs.PixelToWavelengthCoeffs[0]), 
					&pResults->wavelength[i]);
			if (ret_val < 0)
			{
