...
*/
DLPSPEC_ERR_CODE dlpspec_scan_had_interpret(const uScanData *puScanData, scanResults *pResults, int if_x_shift)
/**
 ...
 */
{
    ...
    */
    ...
    ret_val = dlpspec_scan_had_genPatDef(&cfg, &(pScanData->calibration_coeffs), &patDef, if_x_shift);
    if (ret_val < 0)
        return ret_val;
    ...
    for(i=0; i<patDef.numSets; i++)
    {
        ...
        pHadMatrix = (double*)malloc(sizeof(double) * (patDef.set[i].hadOrder * patDef.set[i].hadOrder + 8));
        pHadMatrixInv = (double*)malloc(sizeof(double) * (patDef.set[i].hadOrder * patDef.set[i].hadOrder + 8));
...
DLPSPEC_ERR_CODE dlpspec_scan_had_genPatDef(const scanConfig *pScanConfig, const calibCoeffs *pCoeffs, patDefHad *patDefH, int if_x_shift)
/**
 ...
 */
{
    ...
    ret_val = dlpspec_scan_col_genPatDef(pScanConfig, pCoeffs, &patDefC, if_x_shift);
    if(ret_val < 0)
    {
        return ret_val;
    }
